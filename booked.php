...
// Insert by property description using guest first and last name, beg_date and end_date (need 2 queries)
if(!($query1 = $mysqli->prepare(
    "INSERT INTO bookings(beg_date, end_date, prop_id, guest_id, total_cost)
        VALUES (?,  ?,
        (SELECT prop_id FROM properties WHERE prop_id = ?),
        (SELECT guest_id FROM guests WHERE fname =  ? && lname =  ?),
        (SELECT DATEDIFF(?, ?) *
        (SELECT daily_cost FROM properties p WHERE p.prop_id = ?)))"))){
    echo "Prepare failed: "  . $query1->errno . " " . $query1->error;
}
if(!($query1->bind_param("ssssssss", $_POST['beg_date'], $_POST['end_date'], $_POST['prop_id'], $_POST['first_name'], $_POST['last_name'], $_POST['end_date'], $_POST['beg_date'], $_POST['prop_id']))){
