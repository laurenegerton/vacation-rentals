...
// CREATE A WISHLIST (`listname, guest_id` is unique - one guest cannot have two wishlist of same name)
if(!($query1 = $mysqli->prepare(
    "INSERT INTO wishlists (listname, guest_id)
        VALUES (?,
        (SELECT guest_id FROM guests WHERE guests.fname = ? && guests.lname = ?))"))){
    echo "Prepare failed: "  . $query1->errno . " " . $query1->error;
}
if(!($query1->bind_param("sss", $_POST['listname'], $_POST['first_name'], $_POST['last_name']))){
